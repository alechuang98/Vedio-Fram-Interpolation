...
def train(model, local_rank):
    ...
    for epoch in range(args.epoch):
        ...
        for i, data in enumerate(train_data):
            ...
            pred, merged_img, loss_l1, loss_flow, loss_ter, flow_mask = model.update(imgs, flow, gt, learning_rate, mul, True)
...
def evaluate(model, val_data, nr_eval, local_rank, writer_val):
    ...
    for i, data in enumerate(val_data):
        ...
        gt = data_gpu[:, 6:9]
        with torch.no_grad():
            pred, merged_img, loss_l1, loss_flow, loss_ter, flow_mask = model.update(imgs, flow, gt, training=False)
