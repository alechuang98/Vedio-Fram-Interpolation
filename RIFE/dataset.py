...
class VimeoDataset(Dataset):
    ...
    def getimg(self, index):
        ...
        flow = (self.flow_data[index]).transpose(1, 2, 0)
        return img0, gt, img1, flow
            
    def __getitem__(self, index):        
        img0, gt, img1, flow = self.getimg(index)
        if self.dataset_name == 'train':
            img0, gt, img1, flow = self.aug(img0, gt, img1, flow, 224, 224)
            ...
            flow = np.zeros((256, 448, 4))
        flow = torch.from_numpy(flow.copy()).permute(2, 0, 1)
        img0 = torch.from_numpy(img0.copy()).permute(2, 0, 1)
        img1 = torch.from_numpy(img1.copy()).permute(2, 0, 1)
        gt = torch.from_numpy(gt.copy()).permute(2, 0, 1)
        return torch.cat((img0, img1, gt), 0), flow